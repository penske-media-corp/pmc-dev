# PMC-DEV
NOTE: This is not the official PMC local development environment.

Local dev proxy, ssl, high level dev stack.

This project is currently being used and maintenance by Hau Vong (hvong@pmc.com) 
...
- Ensure the following are up to date and installed on the host system
    ...
    - Make sure you have setup correct ssh-key access to the required repo,
      - pmc-codesniffer
      - pmc-plugins
      - pmc-vip-go-plugins
...
## Multi wp projects development

- Start up the services (Order of commands execution are impportant)
    1. Provision & start up the services 
        - `./dev.sh`
    2. Check the wp service start successfully 
...
## Proxy site development for standalone project
...
- Verify the service started properly
    ...
    - Open browser and access https://traefik.pmcdev.local
- Starting a project
    - Update the project docker-compose.yml to include the following label:
    ```
    labels:
      - traefik.enable=true
      - traefik.http.routers.wp-https.entrypoints=websecure
      - traefik.http.routers.wp-https.priority=1
      - traefik.http.routers.wp-https.rule=HostRegexp(`{domain:the-new-project-domain}`)
      - traefik.http.routers.wp-https.tls.certresolver=file
    ```
    - Make sure to setup the service to run under the same network `traefik` 
...
### Issues & PRs
If there's something you don't see support for or needs more work, please feel free to create your own PR
