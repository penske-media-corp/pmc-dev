...
if ( empty( $memcached_servers ) && empty( $redis_server  ) ) {
	return;
}
...
if ( defined( 'WP_BATCACHE' ) && true !== WP_BATCACHE ) {
	return;
}

if ( ! class_exists( 'Jetpack_User_Agent_Info' ) ) {
	if ( file_exists( WP_PLUGIN_DIR . '/jetpack/class.jetpack-user-agent.php' ) ) {
		require_once( WP_PLUGIN_DIR . '/jetpack/class.jetpack-user-agent.php' );
	} else {
		require __DIR__ . '/is-mobile.php';
	}
}
